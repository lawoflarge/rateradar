...
/// 1:1 port of apps/web/src/lib/glossary-terms.ts — single source of truth for
/// glossary terms, used by GlossaryView and GlossaryTermView. Slugs are explicit
/// and stable (don't auto-derive from `term`, so wording can change without
/// breaking routes).
struct GlossaryTerm: Hashable, Identifiable {
    let slug: String
    let term: String
    let def: String
...
enum GlossaryTerms {
    ...
    /// getTerm() from glossary-terms.ts.
    static func term(slug: String) -> GlossaryTerm? {
